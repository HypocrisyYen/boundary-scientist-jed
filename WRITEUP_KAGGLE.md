...
> Illustrated variant of `WRITEUP_FINAL.md` — diagrams as rendered PNGs (for viewers that don't render Mermaid, e.g. Kaggle).
...
**A result we're proud of — reported with its own disconfirming control:** red-team exploration traces, distilled into defender SFT, train a blue defender that **generalizes to novel guardrail-blind attacks** — refusing **16/40** held-out attacks the base model executes (**2/40**), item-clustered CIs non-overlapping, benign utility intact (35/40). The pipeline (red-discovered attack surface → trained defender) works. But our size-matched control tempers the causal story: a generic-safety defender refuses *even more* (32/40) while over-refusing benign (22/40), so the transfer is a property of safety-SFT broadly — red's edge is **utility preservation, not stronger refusal** (§7). That the loop closes at all is why we most want to explore **self-play RL in which red and blue recursively bootstrap each other**.
...
5. What we're proud of anyway: a **memory system + research tree** that lets one LLM reverse-engineer a guardrail purely by experiment, and a **trace-SFT** recipe that teaches an 8B model — which *before training could not structure the scientist role at all* — to run genuine multi-step probe-hypothesize-verify research. And a result we **measured, and stress-tested with a control**: distilling the red scientist's discovered attacks into defender SFT lets the same 8B refuse **16/40 held-out guardrail-blind attacks** the base executes (2/40), benign utility intact. A size-matched generic-safety control refuses even more (32/40) but over-refuses benign (22/40), so the *transfer* is real while the causal advantage of red provenance is narrower — utility preservation, not stronger refusal (§7).
...
*A real meta tree (hard04_inject_h2). Green = the golden UNTRUSTED_TO_ACTION trace; sibling metas collapsed.*
...
- **Self-play RL instead of behavior alignment.** Rather than aligning the agent to a list of safe behaviors, train **red and blue against each other**: red maximizes harm-score, blue maximizes harm-prevented. Our §7 held-out result (red-derived defender 16/40 vs base 2/40 on novel guardrail-blind attacks, benign utility intact) shows the *transfer* is real; the size-matched control (generic safety refuses 32/40 but over-refuses) says the two sides improve from safety-SFT broadly, with red's edge in the utility/safety balance — a model exposed to the space of scoring traces learns *danger and intent as concepts*, not a blocklist, so it is not defeated by a single unseen surface form. The 9,000 s "online episode" framing is already the right shape for this red↔blue loop. (Nail the causality first with the size-matched provenance control, §7.)
